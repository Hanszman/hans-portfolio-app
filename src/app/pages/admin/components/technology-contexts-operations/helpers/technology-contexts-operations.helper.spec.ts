...
} from './technology-contexts-operations.helper';

const technology = (id: string, name: string, slug: string): TechnologyCollectionItemResponse => ({
  id,
  name,
  slug,
  category: 'OTHER',
...
const context = (overrides: Partial<TechnologyContextRecord> = {}): TechnologyContextRecord => ({
  id: 'context-1',
  technologyId: 'tech-1',
...
describe('technology contexts operations helper', () => {
  it('sorts technology options by name', () => {
    expect(
      buildTechnologyContextTechnologyOptions([
        technology('2', 'Zeta', 'zeta'),
        technology('1', 'Angular', 'angular'),
      ]),
    ).toEqual([
      { id: '1', label: 'Angular (angular)', value: '1' },
      { id: '2', label: 'Zeta (zeta)', value: '2' },
    ]);
  });

  it('maps records with translated context labels and scalar fallbacks', () => {
    const translation = {
      instant: jasmine.createSpy('instant').and.returnValue('Professional'),
    } as unknown as TranslationService;
    expect(
      buildTechnologyContextViewModels(
        [context(), context({ id: 'context-2', technology: null, technologyId: 'missing' })],
        translation,
      ),
    ).toEqual([
      {
      ...
      },
      {
        ...
        startedAt: '2026-01-01',
        endedAt: '',
        technologyName: 'missing',
        technologySlug: 'missing',
        contextLabel: 'Professional',
        dateRangeLabel: '01/01/2026 - -',
      },
    ]);
  ...
  it('maps a completed record and preserves a non-empty end date in valid payloads', () => {
    ...
    expect(buildTechnologyContextFormValue(completed)).toEqual({
      technologyId: 'tech-1',
      context: 'PROFESSIONAL',
      startedAt: '2026-01-01',
      endedAt: '2026-12-31',
    ...
    expect(
      buildTechnologyContextFormValue({
        ...
        technology: { id: 'tech-fallback', slug: 'fallback', name: 'Fallback' },
      } as unknown as TechnologyContextRecord),
    ).toEqual({ technologyId: 'tech-fallback', context: '', startedAt: '', endedAt: '' });
    expect(
      buildTechnologyContextFormValue({
        technologyId: undefined,
        technology: null,
      } as unknown as TechnologyContextRecord),
    ).toEqual({ technologyId: '', context: '', startedAt: '', endedAt: '' });
  });
  ...
  it('formats dates for operation summaries and preserves invalid values', () => {
    ...
    expect(
      buildTechnologyContextViewModels(
        [context({ context: '' as never, technology: null, technologyId: 'fallback' })],
        translation,
      )[0].contextLabel,
    ).toBe('-');
  });
