# Plano de Implementação — `hans-portfolio-app` (v2)

> Objetivo: guiar a execução do remake do portfólio antigo (`victor_hanszman_portfolio-old`) no repositório novo (`hans-portfolio-app`), usando **Angular 20.3.6** no front-end, **`hans-ui-design-lib` via CDN/web components** como base visual, e **backend próprio em .NET/C# + PostgreSQL (Neon)** desde a primeira etapa.
>
...
> - a `hans-ui-design-lib` e seu padrão de consumo em Angular via CDN;
...
### 2.2. Backend — .NET simples de entender, mas robusto

O backend deve ser construído de forma **didática**, já que o projeto também servirá como estudo/aprofundamento em C#/.NET, mas sem perder robustez arquitetural.
...
- **Swagger / OpenAPI** desde o início.
...
- Angular, TypeScript e React como stack principal
...
- criação dos projetos `Api`, `Application`, `Domain`, `Infrastructure`
...
- Tooltip
- Toggle / Switch
- eventualmente Table, se a lib já tiver ou vier a ter

## 11.2. Sobre `Accordion`

Você perguntou o que seria.

### Definição prática
Accordion é um componente de **expansão/colapso de blocos**, tipo:
- clica no título da seção
- abre o conteúdo abaixo
- pode ter um item aberto por vez ou vários

### Onde ele faria sentido no remake
- FAQs ou explicações secundárias
- experiências com detalhes expansíveis
- seções de “mais informações” em projetos
- painel admin com agrupamentos

### Decisão
**Não é obrigatório para a primeira entrega do portfólio.**
Mas **faz sentido propor sua criação na `hans-ui-design-lib`** como evolução futura, porque é componente reutilizável de verdade.

#### Se for criado na lib, seguir o padrão já definido
- Clean Code
- DRY
- SRP
...
- 6 arquivos padrão:
  ...
  - `mdx`
  - `stories`
- helpers e testes auxiliares quando necessário
...
Faz sentido, sim, pensar nisso como componente reutilizável da lib.

### Recomendação
Criar futuramente um **Language Selector** na `hans-ui-design-lib` **se ele for realmente reaproveitado nos outros projetos**.

### Abordagem sugerida
A lib fornece:
- estrutura visual do selector
- exibição dos idiomas disponíveis
- ícone/bandeira opcional
- estados de hover/selected/disabled
- eventos de mudança

Cada projeto fornece:
- a lista de idiomas que suporta
- os textos de tradução
- a lógica específica da engine de tradução

Ou seja: o componente da lib **não acopla à biblioteca de tradução**.
Ele só representa a interface de troca de idioma.
